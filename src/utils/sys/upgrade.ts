...
import { fetchVersion } from '@/api/system/manage'
...
export async function systemUpgrade(): Promise<void> {
  fetchVersion().then((res: Api.SystemManage.VersionInfo) => {
    let content = res.content ? res.content : ''
    content = content.replace('；', ';')
    upgradeLogList.value.push({
      version: res.versionNum,
      title: res.title,
      date: res.releaseTime,
      detail: content.split(';'),
      requireReLogin: res.reLogin,
      remark: res.remark
    })
    versionManager.processUpgrade()
  })
}
