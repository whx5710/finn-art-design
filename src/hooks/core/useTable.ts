...
function useTableImpl<TApiFn extends (params: any) => Promise<any>>(
  ...
  const pagination = reactive<Api.Common.PaginationParams>({
    pageNum: ((searchParams as Record<string, unknown>)[pageKey] as number) || 1,
    ...
    try {
      ...
      if (useCache && cache) {
        ...
        if (cachedItem) {
          ...
          pagination.total = cachedItem.response.total ?? pagination.total ?? 0

          // 修复：避免重复设置相同的值，防止响应式循环更新
          const paramsRecord = searchParams as Record<string, unknown>
          if (paramsRecord[pageKey] !== pagination.pageNum) {
            paramsRecord[pageKey] = pagination.pageNum
          }
      ...
      pagination.total = standardResponse.total ?? pagination.total ?? 0
  ...
  const refreshRemove = async (): Promise<void> => {
    ...
    // 如果当前页为空且不是第一页，回到上一页
    if (data.value.length === 0 && pageNum > 1) {
      pagination.pageNum = pageNum - 1
      ;(searchParams as Record<string, unknown>)[pageKey] = pageNum - 1
      await getData()
    }
  }
